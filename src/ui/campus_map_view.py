...
def open_campus_map_dialog(app) -> None:
    ...
    def render_building_position_row(building: Building) -> None:
        ...
        with ui.row().classes("w-full items-center gap-3 py-1 border-b border-gray-100 dark:border-gray-700"):
            ...
            ui.number(label=t("campus_map.column.x"), value=building.position[0], step=1, on_change=on_x_change).props("dense outlined").classes("w-32")
            ui.number(label=t("campus_map.column.y"), value=building.position[1], step=1, on_change=on_y_change).props("dense outlined").classes("w-32")
            if has_geometry:
                # debounce : chaque frappe déclencherait sinon une mise à
                # l'échelle intermédiaire (« 2 » avant « 25 ») du bâtiment.
                ui.number(label=t("campus_map.column.width"), value=width, step=1, min=0.1, on_change=on_width_change).props("dense outlined debounce=600").classes("w-32")
                ui.number(label=t("campus_map.column.depth"), value=depth, step=1, min=0.1, on_change=on_depth_change).props("dense outlined debounce=600").classes("w-32")
            else:
                ui.label(t("campus_map.no_polygon")).classes("w-[16.5rem] text-xs text-gray-500 dark:text-gray-300")
    ...
    def rows_view() -> None:
        ...
        if not shown:
            ui.label(t("campus_map.search.no_match")).classes("text-gray-500 dark:text-gray-300 py-4")
    ...
    with ui.dialog().props("maximized") as dialog, ui.card().classes("w-full h-full"):
        with ui.row().classes("items-center justify-between w-full mb-2"):
            ui.label(t("campus_map.dialog.title")).classes("text-lg font-semibold")
            ui.button(icon="close", on_click=dialog.close).props("flat round")

        if not app.campus.buildings:
            ui.label(t("campus_map.empty")).classes("text-gray-500 dark:text-gray-300")
        else:
            ui.label(t("campus_map.preview_caption")).classes("text-xs text-gray-500 dark:text-gray-300 mb-1")
            with ui.element("div").style("width: 100%; height: 40vh;"):
                container_ref["el"] = ui.column().classes("w-full h-full")

            ui.separator().classes("my-3")

            ui.label(t("campus_map.table_caption")).classes("text-xs text-gray-500 dark:text-gray-300 mb-1")
            ui.input(
                label=t("campus_map.search"),
            ...
            with ui.row().classes("w-full items-center gap-3 pb-2 border-b-2 border-gray-300 dark:border-gray-600 text-sm font-semibold text-gray-500 dark:text-gray-300"):
                ui.label(t("campus_map.column.building")).classes("flex-1")
                ui.label(t("campus_map.column.x")).classes("w-32")
                ui.label(t("campus_map.column.y")).classes("w-32")
                ui.label(t("campus_map.column.width")).classes("w-32")
                ui.label(t("campus_map.column.depth")).classes("w-32")
